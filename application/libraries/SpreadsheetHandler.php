				...
				foreach ($project->items as $k => $item)
				{
					...
					switch ($item->item_type)
					{
						...
						case 'Physical';
							...
							$this->setPhysicalHistoryRow($activeSheet, $row, $alphabet[$k], $item->name, number_format($item->budget_ceiling, 2, ',', '.'), $project->provider->name, '100', '100', number_format($realization, 2, ',', '.'), $target, $planning, $percentageRealization, $percentageRealization - $planning, number_format($item->budget_ceiling - $realization, 2, ',', '.'));
